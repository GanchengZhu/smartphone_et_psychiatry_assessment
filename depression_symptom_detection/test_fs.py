...
def test(train_X, train_y, test_X, test_y, args):
    ...
    top_indices = select_top_n_by_mi(train_X, train_y, args.n_feature_keep)
    train_X = train_X[:, top_indices]
    test_X = test_X[:, top_indices]
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(s_k_fold.split(train_X, train_y)):
        ...
        y_proba_val = pipeline.predict_proba(val_X_fold)[:, 1] if hasattr(classifier, 'predict_proba') else [0] * len(
            val_X_fold)
